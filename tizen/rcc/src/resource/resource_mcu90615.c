...
/*				resource_DHT11 - GPIO				*/
/*
void resource_close_dht11(int pin_num)
{
...
int resource_read_dht11(int pin_num)
{
	int ret = PERIPHERAL_ERROR_NONE;
	int value;
	...
	if(!resource_get_info(pin_num)->opened){
		peripheral_gpio_open(pin_num, &resource_get_info(pin_num)->sensor_h);

		//gpio_setting
		//peripheral_gpio_set_direction(&gpio_h, PERIPHERAL_GPIO_DIRECTION_OUT_INITIALLY_HIGH);
		//peripheral_gpio_set_edge_mode(&gpio_h, PERIPHERAL_GPIO_EDGE_BOTH);

		//gpio_read
		peripheral_gpio_read(&resource_get_info(pin_num)->sensor_h, &value);
